...
app.get('/statistics/overview', async (req, res) => {
  try {
    const movies = await Movie.find({}, { _id: 0 });
...
app.post('/delete-movie/:title', async (req, res) => {
  try {
...
app.get('/edit-movie/:title', async (req, res) => {
  try {
...
app.post('/update-movie/:title', async (req, res) => {
  try {
    const movieTitle = req.params.title;
...
// Ứng dụng thứ hai (đăng nhập và đăng ký)
